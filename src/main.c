...
void init_signal(void)
{
    ...
    if (sigemptyset(&tstp_alrm_mask) == -1)
        perror_exit("tstp_alrm_mask sigemptyset");
    if (sigaddset(&tstp_alrm_mask, SIGTSTP) == -1)
        perror_exit("tstp_alrm_mask sigaddset SIGTSTP");
    if (sigaddset(&tstp_alrm_mask, SIGALRM) == -1)
        perror_exit("tstp_alrm_mask sigaddset SIGALRM");
    ...
    if (sigemptyset(&sa.sa_mask) == -1)
        perror_exit("sa.sa_mask sigemptyset");
    if (sigaddset(&sa.sa_mask, SIGTSTP) == -1)
        perror_exit("sa.sa_mask sigaddset tstp");
    if (sigaddset(&sa.sa_mask, SIGALRM) == -1)
        perror_exit("sa.sa_mask sigaddset alrm");

    if (sigaction(SIGTSTP, &sa, NULL) == -1)
        perror_exit("sigaction SIGTSTP");
    if (sigaction(SIGALRM, &sa, NULL) == -1)
        perror_exit("sigaction SIGALRM");
}
